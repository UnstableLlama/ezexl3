# Chat prompt format templates for exllamav3 models.
#
# Vendored and adapted from exllamav3 examples/chat_templates.py
# Original author: turboderp (https://github.com/turboderp-org/exllamav3)
# License: MIT (same as exllamav3)
#
# These templates define how conversation history is formatted for each model
# family before being sent to the model for inference.
...
class PromptFormat_seed(PromptFormat):
    ...
    def default_system_prompt(self, think):
        if think:
            ...
            ns_interval = budget_reflections_v05[
                min([k for k in budget_reflections_v05 if k >= self.thinking_budget])
            ]
            return (
