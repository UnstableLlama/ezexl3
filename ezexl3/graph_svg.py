...
def make_plot(bpw, kld, ppl, gib, title, outfile, add_checks=True):
    ...
    # Reduce bottom-axis label density by ~50% for cleaner readability.
    bottom_ticks = ax.get_xticks()
    if len(bottom_ticks) >= 4:
        reduced_ticks = bottom_ticks[::2]
        if reduced_ticks[-1] != bottom_ticks[-1]:
            reduced_ticks = np.append(reduced_ticks, bottom_ticks[-1])
